...
function init() {
  printToConsole('Green Acres module initializing..');
  setStatusIconBusy(true);
  loadOmniTheme();
}

function findTargets() {
  // Select all property cards
  cards = document.querySelectorAll(SELECTORS.card);
...
function processProperty(property) {
  ...
  infoButton.onclick = (e) => {
    ...
    if (additionalInfo !== null) {
      infoButton.setAttribute('data-additional-info', additionalInfo);
      infoButton.style.backgroundColor =
        additionalInfo.trim() === '' ? 'transparent' : COLORS.green;
      infoButton.innerText = additionalInfo.trim() === '' ? '+' : '✓';
    }
  };
...
setTimeout(() => {
  ...
  if (!properties || properties.length === 0)
    return setStatusIconBusy(false);

  // Iterate over each property to inject buttons
  processProperties();
  setStatusIconBusy(false);
